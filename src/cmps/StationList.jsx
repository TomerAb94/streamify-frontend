...
import { setTracks, setCurrentTrack, setCurrentStationId, setIsPlaying } from '../store/actions/track.actions.js'
import { youtubeService } from '../services/youtube.service.js'

export function StationList({ stations, playlist, onAddStation, onRemoveStation, onUpdateStation }) {
  ...
  function filterStations() {
    ...
    if (filterBy.txt) {
      ...
      filteredStations = filteredStations.filter((station) => regex.test(station.title))
    }
      ...
      const playlistQueue = await Promise.all(
        station.tracks.map(async (track, index) => {
          return {
            ...track,
            nextId:
              index < station.tracks.length - 1 ? station.tracks[index + 1].spotifyId : station.tracks[0].spotifyId,
            prevId:
              index > 0 ? station.tracks[index - 1].spotifyId : station.tracks[station.tracks.length - 1].spotifyId,
  ...
  return (
    <section className="station-list-container" onClick={() => setActiveStationId(null)}>
      <header>
        <h3>Your Library</h3>
      ...
      <form className="station-list-form">
        <div className="search-bar">
          ...
          <div className="input-container">
            <SvgIcon iconName="search" className="search-icon" />
            <input type="text" placeholder="Search in Your Library" onInput={handleInput} ref={searchInputRef} />
          </div>
          ...
          <div className={`context-menu ${isSortMenuOpen ? 'open' : ''}`} ref={sortMenuRef}>
            <SortMenu setFilterBy={setFilterBy} />
          </div>
        </div>

        {stationsToShow.length === 0 && loggedInUser && (
        ...
        {stationsToShow.length === 0 && !loggedInUser && (
          <div className="no-stations">
            <p>Signup or logIn to create your own playlists</p>
          </div>
        )}
        ...
        <ul className="station-list">
          {stationsToShow.map((station) => (
            <NavLink to={`/station/${station._id}`} key={station._id}>
              <li
                className={`station-preview ${clickedStationId === station._id ? 'clicked' : ''}`}
                onClick={() => handlePlayClick(station._id)}
                onContextMenu={(ev) => toggleActionMenu(ev, station._id)}
              >
                <StationPreview station={station} onPlayStation={onPlayStation} onPause={onPause} />
              </li>
            </NavLink>
          ))}
        </ul>
